...
Router#
```
**router power off then onn again**
```
...
##############
```
[**when booting press control+c for windows for other  link is [link](https://www.cisco.com/c/en/us/support/docs/routers/10000-series-routers/12818-61.html)**]
```
...
rommon 2 > help ?
monitor: command "help ?" not found
rommon 3 > help 
boot                boot up an external process
confreg             configuration register utility
dir                 list files in file system
...
Directory of flash:/
    ...
    1  -rw-     4670455          <no date>  2960-lanbasek9-mz.150-2.SE4.bin
    2  -rw-     4670455          <no date>  c2960-lanbasek9-mz.150-2.SE4.bin
    3  -rw-        1126          <no date>  config.text

64016384 bytes total (54674348 bytes free)
Switch>ena
Switch>enable 
Switch#show versio
Switch#show version 
...
Switch#
```
# পেনড্রাইভে ব্যাকআপ সুইচের ISO
সুইচের ISO (যেমন Cisco switch) পেনড্রাইভে ব্যাকআপ করতে হলে নিচের ধাপগুলো অনুসরণ করুন:

### ১. **USB ড্রাইভ সুইচে লাগান**
   - আপনার পেনড্রাইভটি সুইচের USB পোর্টে সংযুক্ত করুন (যদি আপনার সুইচ মডেল USB ড্রাইভ সমর্থন করে)।

### ২. **চেক করুন সুইচ USB ড্রাইভ সঠিকভাবে সনাক্ত করছে কিনা**
   - সুইচে USB ড্রাইভ সনাক্ত হয়েছে কিনা চেক করতে নিচের কমান্ডটি ব্যবহার করুন:
   ```bash
   dir usb0:
   ```
   - যদি USB ড্রাইভ সঠিকভাবে সনাক্ত হয়, তাহলে এটি ড্রাইভের কন্টেন্ট দেখাবে। যদি না হয়, তাহলে USB ড্রাইভটি সঠিকভাবে সংযুক্ত আছে কিনা নিশ্চিত করুন বা অন্য পোর্ট ব্যবহার করে দেখুন।

### ৩. **ISO ফাইল USB ড্রাইভে কপি করুন**
   - এখন সুইচের ISO ইমেজ ফাইল (যেমন Cisco IOS ইমেজ) পেনড্রাইভে কপি করার জন্য `copy` কমান্ড ব্যবহার করুন। কমান্ডের সঠিক রূপ হবে:
   ```bash
   copy flash: <filename.iso> usb0:/
   ```
   - `<filename.iso>` এর স্থানে সেই ISO ফাইলের নাম দিন যেটি সুইচের ফ্ল্যাশ মেমরিতে (অথবা স্টোরেজ ড্রাইভে) আছে।

   উদাহরণস্বরূপ, যদি ISO ফাইলের নাম `c2960x-universalk9-mz.152-7.E3.bin` হয়, তাহলে কমান্ড হবে:
   ```bash
   copy flash: c2960x-universalk9-mz.152-7.E3.bin usb0:/
   ```

### ৪. **কপি সফলভাবে হয়েছে কি না যাচাই করুন**
   - কপি প্রক্রিয়া সম্পন্ন হওয়ার পরে, আপনি USB ড্রাইভটি চেক করতে পারেন যে ISO ফাইলটি সঠিকভাবে কপি হয়েছে কিনা। চেক করার জন্য নিচের কমান্ডটি ব্যবহার করুন:
   ```bash
   dir usb0:
   ```
   - এটি USB ড্রাইভের সকল ফাইল দেখাবে, যাতে আপনি কপি করা ISO ফাইল দেখতে পাবেন।

### ৫. **USB ড্রাইভ নিরাপদভাবে বের করুন**
   - একবার ব্যাকআপ সম্পন্ন হলে, USB ড্রাইভটি সুইচ থেকে নিরাপদভাবে বের করুন। পেনড্রাইভটি সরিয়ে নিতে পারেন।

### টিপস:
- **USB পোর্টের নাম**: অধিকাংশ Cisco সুইচে প্রথম USB পোর্টটি `usb0:` নামে পরিচিত। যদি একাধিক USB পোর্ট থাকে, তাহলে আপনাকে `usb1:`, `usb2:` ইত্যাদি ব্যবহার করতে হতে পারে।
- **ফাইল সিস্টেমের সামঞ্জস্যতা**: নিশ্চিত করুন আপনার USB ড্রাইভ FAT16 অথবা FAT32 ফাইল সিস্টেমে ফরম্যাট করা। সুইচ NTFS বা অন্য কোন ফাইল সিস্টেম সাপোর্ট নাও করতে পারে।

এই ধাপগুলো অনুসরণ করে আপনি সহজেই সুইচের ISO ফাইল পেনড্রাইভে ব্যাকআপ নিতে পারবেন।
